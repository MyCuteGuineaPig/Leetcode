...
public:
    int findIntegers(int num) {
        static int fb[30] = { 3, 5, 8, 13, 21, 34, 55, 89, 144, 233, 377, 610, 987, 1597, 2584, 4181, 6765, 10946,
        17711, 28657, 46368, 75025, 121393, 196418, 317811, 514229, 832040, 1346269, 2178309, 3524578 };
        if (num < 3) 
            return num + 1; 
        if (num == 3 )
        ...
        for(int i =  29; i>=2; --i){
            if (num & (1<<i)){
                /*
                num & (1<<i)   num & (1 << (i-1)) 假如num有n位，表示任何n位不连续的1的数都可以用

                num & (1<<i) but not num & (1 << (i-1)), 只能先用n-1的 位数的所有可能性 + 递归找后面的
                 */
                return num & (1 << (i-1)) ? fb[i-1] : fb[i-2] + findIntegers(num& ~(1<<i));
            }
        }
        return 1;
    }
