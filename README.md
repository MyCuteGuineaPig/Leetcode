...
| [257	Binary Tree Paths](https://leetcode.com/problems/binary-tree-paths/description/) | 	O(n * h) | O(h) | Easy 	| -可以用queue，BFS 扫每个level，返回最右侧的 -也可以用加上一个null pointer在 每个levle queue 最后面，一旦返回null，知道这层扫完了，append last number |
| [282	Expression Add Operators](https://leetcode.com/problems/expression-add-operators/)	| O(4^n) | O(n) | Hard | DFS，需要cv (cumulative sum), pv(previous sum), pos 到现在process的index，注意: -现在是乘号, cv = cv - pv + p*n, pv = pv*n -现在是剑豪, cv = cv - pv + n, pv = -n |
