...
| [373. Find K Pairs with Smallest Sums](https://leetcode.com/problems/find-k-pairs-with-smallest-sums/) | _O(k * log(min(n, m, k)))_ | _O(min(n, m, k))_	| Medium | 🔍注: 不用hashset, 可不重复pus 进heap <br/> 🎅不能用two pointer, 比如```[1,7], [2,6]```, 结果是```[1,2],[1,6],[2,7]```, two pointer给的是```[1,2],[1,6],[6,7]``` |
