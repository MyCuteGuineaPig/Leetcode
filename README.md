...
| [818. Race Car](https://leetcode.com/problems/largest-sum-of-averages/)	|	_O(nlogn)_	|	_O(n)_|	Hard | if t=2^n-1, dp\[t]=n, else dp\[t] = min(n + 1 + dp\[(1<<n)-1 - i], n-1+2+k+dp\[i-((1<<(n-1)) - (1<<k))] for j in \[0,n] ), n + 1 + dp\[(1<<n)-1 - i]是先到2^n-1的是n, +1是做reverse(停)，dp\[(1<<n)-1 - i], n-1+2+k+dp\[i-((1<<(n-1)) - (1<<k))] 是先到之前2^(n-1)-1的点,然后往回走(2^k-1)点，再从那个点到t |
