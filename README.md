...
| [514. Freedom Trail](https://leetcode.com/problems/freedom-trail/description/)	|	_O(k) ~ O(k \* r^2)_	|	_O(r)_|	Hard | 记录ring每个字母的位置，字母位置push进vector, loop key,把i-1的字母vector作为起始点，把key\[i]字母vector作为现在点，找到i位置最小移动距离，如果ring\[k] = key\[i], update dp\[k]的值 |
