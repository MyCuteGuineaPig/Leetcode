...
sliding windows: windows都是看以当前字母结尾的window. 比较对象```s1```, 被比较对象```s2``` <ul><li>可以记录当前substring的开始位置，</li><li>用数字记录substring的长度 </li><li>用hashset和两个pointer记录当前windows的长度 </li><li> map+pointer 1 map + 2 pointers: map先记录比较对象 ```map[s1[i]]++```, 再对被比较对象 __所有字母__ / __key出现__  相加减, ```map[s2[i]]``` 
<ul> <li> **固定windows 长度**, <ul> <li> 一个pointer ```count```(初始值为 ```count=len```), 它的值会变动, 表示固定windows 内多少个满足条件 (满足条件 ```if --map[s2[i]] >= 0 ```, ```s2```现在windows中在```s1```中出现过, ```--count```),  <li/><li> 一个pointer ```len```(不变化)表示比较对象长度,用来移动窗口, ```if i>=len-1```, ```map[s2[i-len+1]]++``` </li><li>当```count == 0``` 表示windows终点 </li> </ul>
<li> **不固定长度**. <ul> <li> 一个pointer```left```(变化) , </li> <li> 一个pointer ```len``` 记录```s1```长度(不变化) </li>  <li>**比较条件**: ```if(map[s2[i]])<0 ``` 表示```s2[i]```现windows中个数大于```s1```中个数或者在```s1```中没有出现, 如果if true, 则移动窗口, 下面两种方式都可以 <ul><li> 方式一: ```while(left <= i && map[s2[i]]< 0) map[s2[left++]]++```,```map[s[2]]< 0``` 小于0原因是 不在```s1```中或者 减多了, e.g.1 ```s1=abc```, ```s2=ababc```, 在index=2, 第二个a, 有两个a 多于s1中个数, e.g. 2 ```s1=abc```, ```s2=abdabc```, 在index=2, d在s1中没有出现 ) </li> <li>```while ++left until > i```  当window ```i - j = len -1```, ```j```是起点, </li> </li> </ul>  </li>  </ul> 
<li>  可以用两个map,一个map记录比较对象(T)，一个记录被比较对象(S), 还需要一个count记录S中T出现的个数, start记录windows起始点, 初始化count = len(T); </br>只有当sdict\[s\[i]] < tdict\[s\[i]], count--; 当count == 0, 满足情况,append to res; </br>移动窗口过程中,dict\[s\[start]]--, start++,只有当sdict\[s\[start]] < tdict\[s\[start]]时 ++count, </br>比如[30题 Substring with Concatenation of All Words](https://leetcode.com/problems/substring-with-concatenation-of-all-words/description/), [76题 Minimum Window Substring](https://leetcode.com/problems/minimum-window-substring/)</br>两个题区别是30不能包括多余的string (不可以sdict\[s\[start]] > tdict\[s\[start]]), 76是允许的 </li>  </ul> 
